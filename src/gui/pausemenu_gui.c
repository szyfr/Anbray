...
void DrawPauseMenu(Gamestate *gamestate) {
    if (gamestate->pmFlags & (1 << 0) && gamestate->state == STATE_MAP) {
        u32 top    = (gamestate->optionsData->resolutionHeight/2)-150;
        u32 bottom = (gamestate->optionsData->resolutionHeight/2)+150;
        u32 left   = (gamestate->optionsData->resolutionWidth/2) -100;
        u32 right  = (gamestate->optionsData->resolutionWidth/2) +100;
        
        GuiPanel((Rectangle){left, top, 200, 300});
        ...
        // Quit
        if(GuiButton((Rectangle){left+10, top+250, 180, 40}, gamestate->coreLocalization[2])) {
            EndDrawing();
            exit(0);
        }
    }
